...
ofstream file("tas-me-output.txt"); 
//lock function to prevent other process to enter cs
void lock() {
    while (flag.exchange(true)) {}
}
//to restore flag to false
void unlock() {
    flag.store(false);
}
//to get system Time
...
//this function runs on each thread
void testCS(int id){
    vector<string> vi;
    //generationg exponential distribution with mean lambda1, lambda2
    ...
    //storing strings that are to be printed in output
    arr[id]=vi;
}